...
const CORRIDORS = [
  { slug: "africa-canada", title: "Africa ↔ Canada", headline: "The Primary Cross-Border Infrastructure Corridor", desc: "The flagship corridor connecting all African markets with Canada — payments, logistics, compliance, and advisory for the full continent.", stats: [{ label: "Bilateral Trade", value: "$4B+" }, { label: "Diaspora in Canada", value: "500K+" }], color: "#C5A059", tag: "Primary Corridor" },
  ...
  { slug: "nigeria-canada", title: "Nigeria ↔ Canada", headline: "Africa's Largest Economy Connected to Canada", desc: "Africa's most populous nation with a dynamic fintech ecosystem and strong bilateral trade with Canadian markets.", stats: [{ label: "Population", value: "200M+" }, { label: "Currency", value: "NGN/CAD" }], color: "#3b82f6", tag: "Active" },
  { slug: "kenya-canada", title: "Kenya ↔ Canada", headline: "East Africa's Financial Hub", desc: "Kenya's leadership in mobile money and fintech makes it the strategic entry point for Canadian businesses in East Africa.", stats: [{ label: "Population", value: "55M+" }, { label: "Currency", value: "KES/CAD" }], color: "#8b5cf6", tag: "Active" },
];
    ...
    <>
      <section className="bg-blue-950 pt-32 pb-20 relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          ...
          <p className="text-xl text-white/60 max-w-2xl leading-relaxed mb-10">CorridorBridge operates across four active Africa-Canada trade lanes — providing payments, compliance, logistics, and advisory infrastructure for each corridor.</p>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {CORRIDORS.map(({ slug, title, headline, desc, stats, color, tag }) => (
              <Link key={slug} href={`/corridors/${slug}`}
                className="bg-white border-2 border-gray-200 rounded-2xl p-8 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group block"
                ...
                <h3 className="font-display text-2xl font-bold mb-1" style={{color}}>{title}</h3>
